...
Minimal robustness matrices are now defined in the same runner:
`k1000_minimal`, `k5000_minimal`, and combined `k1000_k5000_minimal`. These
reuse the K=3000 physical/Rayleigh setup with only the device count changed and
four roles per density: member-quota baseline, capped-quota pure-ALOHA
